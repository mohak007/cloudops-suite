...
def ec2_page():
    ...
    if operation == "Create Instances":
        ...
        instance_name = st.text_input("Instance Name", value="dev-server")

        instance_type = st.selectbox("Instance Type", ["t2.micro", "t3.micro"])

        security_group_name = st.text_input("Security Group Name", value="cloudops-sg")

        key_pair_name = st.text_input("Key Pair Name", value="cloudops-key")

        ports = st.text_input("Ports (comma separated)", value="22,80")
        ...
        if st.button("Fetch Instances"):
            ...
            response = get("/ec2/list-instances")
        ...
        if st.button("Start"):
            ...
            with st.spinner("Starting instance..."):
                ...
                response = get(f"/ec2/start-instance/{instance_id}")
        ...
        if st.button("Stop"):
            ...
            with st.spinner("Stopping instance..."):
                ...
                response = get(f"/ec2/stop-instance/{instance_id}")
        ...
        if st.button("Terminate"):
            ...
            with st.spinner("Terminating instance..."):
                ...
                response = delete(f"/ec2/terminate-instance/{instance_id}")
