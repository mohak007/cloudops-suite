...
def docker_page():
    ...
    if operation == "Build Image":
        ...
        if st.button("Start Container"):
            ...
            st.json(
                post(
                    f"/docker/start-container/{container_id}"
                )
            )
        ...
        if st.button("Stop Container"):
            ...
            st.json(
                post(
                    f"/docker/stop-container/{container_id}"
                )
            )
        ...
        if st.button("Remove Container"):
            ...
            st.json(
                delete(
                    f"/docker/remove-container/{container_id}"
                )
            )
        ...
        if st.button("Remove Image"):
            response = delete(
            f"/docker/remove-image/{image_name}"
        )
