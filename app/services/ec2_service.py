...
def create_ec2_instance(data: EC2CreateRequest):
    try:
        ...
        security_group_id = create_security_group(data.security_group_name, data.ports)
            ...
            TagSpecifications=[
                {
                    ...
                    "Tags": [{"Key": "Name", "Value": data.instance_name}],
                }
            ],
        )
        ...
        logger.info(f"waiting for EC2 instance to enter" f"running state{instance_id}")
...
def list_ec2_instances():
    ...
    try:
        ...
        logger.error(f"Error fetching instances: " f"{str(error)}")
...
def stop_ec2_instance(instance_id: str):
    try:
        ...
        return {"message": f"Stopping instance {instance_id}"}
    except ClientError as error:
...
def terminate_ec2_instance(instance_id: str):
    ...
    try:
        ...
        logger.info(f"Terminate request submitted for " f"{instance_id}")
        ...
        logger.error(f"AWS ClientError while terminating " f"instance: {str(error)}")
        raise AWSOperationException(str(error))
        ...
        logger.error(f"Unexpected error while terminating " f"instance: {str(error)}")
...
def create_security_group(group_name: str, ports: list):
    ...
    try:
        ...
        logger.info(f"Checking if security group exists: " f"{group_name}")
        ...
        if existing_groups["SecurityGroups"]:
            ...
            logger.info(f"Using existing security group: " f"{security_group_id}")
        ...
        response = ec2_client.create_security_group(
            GroupName=group_name, Description="Security group for CloudOps Suite"
        )

        security_group_id = response["GroupId"]

        logger.info(f"Created security group: " f"{security_group_id}")
        ...
        logger.info("SSH port 22 opened successfully")

        return security_group_id

    except ClientError as error:

        logger.error(f"AWS ClientError while handling SG: " f"{str(error)}")
        raise AWSOperationException(str(error))
        ...
        logger.error(f"Unexpected error while handling SG: " f"{str(error)}")
...
def create_key_pair(key_name: str):
    ...
    try:
        ...
        logger.info(f"Checking if key pair exists: {key_name}")
        ...
        if "InvalidKeyPair.NotFound" in str(error):
            ...
            logger.info("Key pair not found. Creating new key pair.")
            ...
            logger.info(f"Created key pair and saved PEM: " f"{pem_file}")

            return key_name

        logger.error(f"Key pair error: {str(error)}")
        raise AWSOperationException(str(error))
        ...
        logger.error(f"Unexpected key pair error: " f"{str(error)}")
